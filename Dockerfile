...
WORKDIR /bfe-register
COPY . .
RUN CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -ldflags "-X main.version=`cat VERSION`"

FROM alpine:3.15 AS run
RUN apk update && apk add --no-cache ca-certificates
COPY --from=build /bfe-register/bfe /bfe-register/bin/
COPY conf /bfe-register/conf/
EXPOSE 8080 8443 8421

WORKDIR /bfe/bin
ENTRYPOINT ["./bfe-register"]
CMD ["-c", "../conf/bfe-register.conf", "-l", "../log"]
